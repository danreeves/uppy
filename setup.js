...
try {
  ...
  const kvResult = execSync('npx wrangler kv:namespace create UPTIME_KV', {
    encoding: 'utf8',
  });
  const kvPreviewResult = execSync(
    'npx wrangler kv:namespace create UPTIME_KV --preview',
    { encoding: 'utf8' }
  );
  ...
  if (kvIdMatch && kvPreviewIdMatch) {
    ...
    console.log(
      '❌ Failed to extract KV namespace IDs. Please update wrangler.toml manually.'
    );
  ...
  console.log(
    '3. Create KV namespaces: npm run kv:create && npm run kv:create-preview'
  );
